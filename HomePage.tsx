...
function HomePage() {
  ...
  async function handleRetirarRecurso(idRecursoSala: number) {
    ...
    if (!confirm('¿Seguro que deseas retirar este recurso?')) return;
    try {
  ...
  return (
    <div className="secretaria-dashboard">
      <aside className={`dashboard-sidebar ${sidebarOpen ? '' : 'dashboard-sidebar-collapsed'}`}>
        ...
        >
          {sidebarOpen ? 'Nueva Reserva' : 'Reservar'}
        </button>
        <button
          type="button"
          className="sidebar-link"
        ...
        >
          {sidebarOpen ? 'Mis Reservas' : 'Reservas'}
        </button>
              ...
              {selectedSala ? (
                selectedSala.recursos.length ? (
                  ...
                  <p className="empty-state">
                    Esta sala aún no tiene recursos tecnológicos asignados.
                  </p>
                )
              ) : (
                <p className="empty-state">
